...
def visualizeData(subset, sub_nums):
    ...
    for i in sub_nums:
        ...
        plt.title('Visualized Image of the Digit {0} \nusing 2D Features - By: Tomoki Koike'.format(i))
        plt.xlabel('x')
        plt.ylabel('y')
        plt.grid(b=True, which='major', color='#666666', linestyle='--')
        plt.minorticks_on()
        plt.grid(b=True, which='minor', color='#999999', linestyle='-', alpha=0.2)
        plt.savefig('visualImage_{0}_2.png'.format(i))
    return
...
def plotAvgSC(K_vals, avgSC_vals):
    ...
    plt.minorticks_on()
    plt.grid(b=True, which='minor', color='#999999', linestyle='-', alpha=0.2)
    plt.savefig('avgSC_vs_K_plot1_2.png')
